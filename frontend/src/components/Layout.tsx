...
const Layout = ({ children }: LayoutProps) => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    // Extract just the numeric part for the API
    const extractNumericPHN = (input: string): string => {
      // Remove "PHN-" prefix if present and any non-numeric characters
      const cleaned = input.replace(/^PHN-/, '').replace(/[^0-9]/g, '');
      return cleaned;
    };

    const numericPHN = extractNumericPHN(raw);
    
    if (!numericPHN) {
      console.error("Invalid PHN - must contain numbers");
      return;
    }

    // Navigate with just the numeric PHN that the API expects
    navigate(`/ward-management?phn=${numericPHN}`);
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <div className="relative w-80">
                    ...
                    <Input
                      type="text"
                      placeholder="Search by PHN (e.g. 24712 or PHN-24712)"
                      value={searchQuery}
